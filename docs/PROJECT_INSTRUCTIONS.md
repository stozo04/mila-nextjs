...
- Chat runtime: `chat-stream` uses Node.js with a 60-second maximum duration; preserve prompt streaming and cancellation.
...
Read the relevant installed guide under node_modules/next/dist/docs/ from the repository root before writing Next.js code; heed breaking changes and deprecations. next.config.ts sets agentRules: false so Next dev does not overwrite the shared-instruction pointers. Keep this guidance current with the installed version.

## Related shared guidance

Read [architecture and UI guidance](ARCHITECTURE_GUIDANCE.md) for the original project prompt's unique conventions. Paths in these documents are relative to the checkout root unless they are Markdown links.

Feature specification review remains required for new product features. This instruction synchronization audit is already authorized and does not introduce a product feature. Explicit approval to prepare a month is handled by prepare-milas-month: preview, then create only the requested missing pair. Verification runs remain read-only and must not call billable routes or submit live writes.
