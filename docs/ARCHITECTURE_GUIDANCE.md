...
- Runtime: Node 22.x is required (see `package.json` `engines`)
...
- `export const runtime = "nodejs"` with maxDuration 60; preserve streaming.
...
Do
- Use Node 22 locally and in CI/CD
...
The route gate, is_mila_admin RPC, and RLS enforce access; route-group names do not. NEXT_PUBLIC_ALLOWED_EMAIL is legacy configuration, not an enforced allowlist. The supported current architecture and environment requirements take precedence over the dated architecture snapshots.
