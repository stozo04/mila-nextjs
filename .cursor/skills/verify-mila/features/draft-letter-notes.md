...
- **Prove the guard first.** Run `node .cursor/skills/verify-mila/control-mila.mjs get /api/blog/test-slug/draft --method POST --body '{"html":"<p>x</p>"}' --expect-unauthorized`. It returns `401` with `{"error":"Sign in to continue."}`.
