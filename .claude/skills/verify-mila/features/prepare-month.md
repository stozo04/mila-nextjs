...
Steven's monthly ritual. A button in the admin banner previews the next milestone to prepare — the most recently completed month, or the month in progress once that one has both its card and letter — its title, dates, shared slug, and letter tag — and, on confirmation, creates that month's journey card together with a private blank letter draft in one atomic operation, then navigates to the new card.
...
- **Prove the guard first.** Run `node .claude/skills/verify-mila/control-mila.mjs get /api/journey/prepare-month --method POST --body '{}' --expect-unauthorized`. It returns `401 Unauthorized` with `{"error":"Sign in to continue."}` — the anonymous caller is rejected before the RPC runs. Do this before touching the UI.
- **Read the live preview headlessly.** Run `node .claude/skills/verify-mila/control-mila.mjs session`, then `... get /api/journey/prepare-month --as-admin --save prepare-month/preview`. Status is `200 OK` with JSON carrying `slug`, `title`, `date`, `section`, `blog_title`, `tag`, `journey_type`, `milestone`, and `period_start`. This exercises `authorizeAdmin()` end to end — session plus the `is_mila_admin` RPC — and runs the same `mila_month_preview` the modal calls, without a browser. It is a read; it creates nothing.
...
- **Open the preview.** Choose **Prepare Mila's Month**. A modal titled `Prepare Mila's Month` opens and loads. It explains that it will "Create the next monthly milestone and a private blank letter draft: the most recently completed month, or the month in progress once that one has its card and letter. If either already exists, nothing is created or changed."
...
- The preview looks ahead exactly one month. Once the most recently completed month has both its journey card and its letter, it shows the month in progress (on 2026-09-23: `three-years-four-months`, "August 30 – September 30, 2026"), never further. If that month is also prepared, the preview stays on it and a submit returns a 409 with "This month already has a journey card or letter. Nothing was created or changed." — a safe no-op, not an error to fix. A partial pair keeps the completed month.
- **Check the preview is not a month that already exists.** If the live preview slug matches an existing card **and** letter while the following month has neither, that is the 2026-09-23 defect (the preview stuck on last month's letter) — report it as a failure.
