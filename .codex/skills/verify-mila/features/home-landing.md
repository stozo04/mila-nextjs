...
- **Load the page anonymously.** Open `/`. Run `node .codex/skills/verify-mila/control-mila.mjs get / --save home-landing/home`. Status is `200 OK`, content-type `text/html`, and no `location` header — the session gate does not fire here.
- **Confirm every slide shipped.** Count the slides in the saved body. Run `(Select-String -Path .codex/skills/verify-mila/artifacts/home-landing/home.html -Pattern 'carousel-item' -AllMatches).Matches.Count`. Bash equivalent: `grep -o carousel-item .codex/skills/verify-mila/artifacts/home-landing/home.html | wc -l`. The count is `14`, matching the fourteen imports in `src/app/(public)/page.tsx`.
...
- **Advance the carousel (browser).** Choose **Next**. In the available browser adapter, open `http://127.0.0.1:3000/`, then click the element with accessible name `Next`. The active slide changes to the following image.
