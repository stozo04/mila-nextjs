...
- **Request robots as a crawler.** Open `/robots.txt`. Run `node .codex/skills/verify-mila/control-mila.mjs get /robots.txt --save site-meta/robots-anonymous`. Observed result is `307 Temporary Redirect` with `location: /login` — **the file is not served to an anonymous client.** Record this; it is the expected-vs-actual finding for this feature.
- **Request the sitemap as a crawler.** Open `/sitemap.xml`. Run `node .codex/skills/verify-mila/control-mila.mjs get /sitemap.xml --save site-meta/sitemap-anonymous`. Same result: `307` to `/login`.
- **Load the privacy policy.** Open `/privacy-policy`. Run `node .codex/skills/verify-mila/control-mila.mjs get /privacy-policy --save site-meta/privacy-policy`. Status is `200 OK` and the body contains the policy text.
- **Follow each legacy redirect.** Run `node .codex/skills/verify-mila/control-mila.mjs get /about/genealogy`. Status is `308 Permanent Redirect` with `location: /`. Repeat for `/index.html` → `/`, `/gender-reveal.html` → `/gender-reveal`, `/my-journey/birthday/birthday.html` → `/my-journey/birthday`, `/my-journey/first-year/my-first-year.html` → `/my-journey/first-year`.
