...
- **Prove the guard first.** Run `node .codex/skills/verify-mila/control-mila.mjs get /api/journey/prepare-month --method POST --body '{}' --expect-unauthorized`. It returns `401 Unauthorized` with `{"error":"Sign in to continue."}` — the anonymous caller is rejected before the RPC runs. Do this before touching the UI.
- **Read the live preview headlessly.** Run `node .codex/skills/verify-mila/control-mila.mjs session`, then `... get /api/journey/prepare-month --as-admin --save prepare-month/preview`. Status is `200 OK` with JSON carrying `slug`, `title`, `date`, `section`, `blog_title`, `tag`, `journey_type`, `milestone`, and `period_start`. This exercises `authorizeAdmin()` end to end — session plus the `is_mila_admin` RPC — and runs the same `mila_month_preview` the modal calls, without a browser. It is a read; it creates nothing.
