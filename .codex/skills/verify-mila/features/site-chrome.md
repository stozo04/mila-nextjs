...
- **Confirm the nav is not server-rendered.** Open `/`. Run `node .codex/skills/verify-mila/control-mila.mjs get / --save site-chrome/home-ssr`, then search the body for `nav-item`. There are **zero** matches and exactly one `navbar-brand`. This is the baseline fact that forces every other check in this file into a browser.
- **See the signed-out nav.** In the available browser adapter, open `http://127.0.0.1:3000/` while signed out and wait for the auth check to settle. The nav shows **Support Mila** and **Sign In** and no section links. Confirm with the DOM, not the accessibility tree: DOM evaluation running `[...document.querySelectorAll('#navbarNav a, #navbarNav button')].map(n => n.textContent.trim())` returns exactly those two. Verified 2026-09-02 against this dev server.
...
- **`read_page` and `find` do not reliably expose this menu's children.** Both report the `<ul>` with no contents while the DOM holds two rendered, visible items. Assert nav contents with DOM evaluation. Trusting the accessibility tree here produced a confident, wrong "the nav is empty" report on 2026-09-02.
