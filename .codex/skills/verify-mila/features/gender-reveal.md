...
- **Confirm the gate.** Open `/gender-reveal` signed out. Run `node .codex/skills/verify-mila/control-mila.mjs get /gender-reveal --save gender-reveal/anonymous`. Status is `307` with `location: /login`.
- **Confirm the legacy redirect.** Run `node .codex/skills/verify-mila/control-mila.mjs get /gender-reveal.html`. Status is `308 Permanent Redirect` with `location: /gender-reveal`.
