...
- **Confirm the anonymous gate.** Open the page signed out. Run `node .codex/skills/verify-mila/control-mila.mjs get /admin/prepare-month-setup --save admin-password-setup/anonymous`. Status is `307` with `location: /login` — the session gate fires before the admin check.
